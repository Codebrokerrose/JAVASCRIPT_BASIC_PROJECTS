...
async function fetchNavigationHTML() {
    ...
    const navPath = navMeta ? new URL(navMeta).pathname : `/${lang}/nav`;

    const response = await fetch(`${navPath}.plain.html`, window.location.pathname.endsWith('/nav') ? {
        cache: 'reload'
    } : {});
...
async function buildMobileMenu(nav) {
    ...
    if (sections && tools) {
        ...
        if (searchItem) {
            ...
            searchItem.append(await createSearchForm({
                type: 'minimal',
                action: searchAction
            }));
        }
...
export default async function decorate(block) {
    ...
    if (navSections) {
        navSections.querySelectorAll(':scope > ul > li').forEach((navSection) => {
            ...
            if (subList) {
                ...
                if (sectionLink) {
                    const sectionLi = createElement('li', {
    ...
    if (navTools) {
        navTools.querySelectorAll(':scope > ul > li').forEach(async (navTool) => {
            ...
            const isLangNav = navTool.querySelector('.fa-globe');
            if (isLangNav && subList) {
                const langRootLength = document.documentElement.lang.length + 1;
                const relPath = window.location.pathname.substring(langRootLength);
                subList.querySelectorAll(':scope > li > a').forEach((langLink) => {
                    langLink.href = `${langLink.getAttribute('href')}${relPath}`;
                });
            }
        });
    }

    const utilRowWrapper = createElement('div', {
        class: 'nav-utility-row'
    });
